...
//                            lj_cutio_dsf.cu
//                             -------------------
//                   GPU kernel for lj/cutio/coul/dsf pair style
//
//  Based on lal_lj_dsf.cu (W. Michael Brown, ORNL).
//  Extended for TBSL: separate qqrd2e_self scaling for DSF self-energy
//  (spair and sself are different ensemble scaling factors).
//
...
__kernel void k_lj_cutio_dsf(const __global numtyp4 *restrict x_,
  ...
  if (ii<inum) {
    ...
    // DSF self-energy uses qqrd2e_self (sself * qqrd2e_base)
    if (EVFLAG && eflag) {
      acctyp e_self = -((acctyp)0.5*e_shift + alpha/MY_PIS) *
        qtmp*qtmp*qqrd2e_self/(acctyp)t_per_atom;
    ...
    for ( ; nbor<nbor_end; nbor+=n_stride) {
      ...
      if (rsq<lj1[mtype].w) {
        ...
        // Pair Coulomb uses qqrd2e (spair * qqrd2e_base)
        if (rsq < cut_coulsq) {
          r = ucl_sqrt(rsq);
...
__kernel void k_lj_cutio_dsf_fast(const __global numtyp4 *restrict x_,
