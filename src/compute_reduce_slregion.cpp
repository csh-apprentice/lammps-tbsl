...
#include "compute_reduce_slregion.h"
...
ComputeReduceSLRegion::ComputeReduceSLRegion(LAMMPS *lmp, int narg, char **arg) :
...
double ComputeReduceSLRegion::compute_one(int m, int flag)
{
  // remove the premmatch to avoid repeated-calculation on our bubble
  //region->prematch();
...
bigint ComputeReduceSLRegion::count(int m)
{
  auto &val = values[m];
